// Editorial source: one page per distinct commercial intent, never per city.
export const services = [
  {
    slug: 'desarrollo-web', name: 'Desarrollo web',
    title: 'Desarrollo web para empresas | Reac Studio',
    description: 'Sitios institucionales, landing pages y webs autogestionables para empresas. Diseño, desarrollo e integraciones con Reac Studio, de forma remota.',
    h1: 'Desarrollo web para empresas que necesitan generar consultas',
    intro: 'En Reac Studio diseñamos y desarrollamos sitios web para explicar tu oferta, facilitar el contacto y conectar la web con tus procesos comerciales. Trabajamos de forma remota con negocios de Argentina y LATAM.',
    fit: 'Conviene empezar por una web cuando tu negocio depende de redes sociales, cuando cuesta entender qué vendés o cuando recibís visitas que no encuentran cómo consultar. Antes de elegir tecnología, definimos qué necesita hacer la persona que llega al sitio y quién va a mantener el contenido.',
    deliverables: [
      ['Estructura y mensaje', 'Ordenamos servicios, páginas, contenidos y llamadas a la acción según el objetivo comercial. La navegación tiene que ayudar a encontrar una respuesta y un siguiente paso claro.'],
      ['Diseño y desarrollo', 'Diseñamos una experiencia responsive y desarrollamos la solución acordada. Revisamos semántica, accesibilidad, metadatos, imágenes y rendimiento para que la web funcione más allá de su apariencia.'],
      ['Contacto y conexiones', 'El alcance puede incluir formularios, WhatsApp e integraciones con herramientas existentes. Definimos cómo llega una consulta al equipo y qué información necesita para responderla.']
    ],
    decision: 'Una landing se concentra en una oferta y una acción, por ejemplo una consulta desde Google Ads. Una web institucional presenta varios servicios, el equipo y proyectos. Una web autogestionable permite que tu equipo actualice contenido; agrega necesidades de capacitación y mantenimiento. Elegimos según el uso real.',
    process: ['Relevamos objetivo, público, contenido disponible y herramientas actuales.', 'Acordamos alcance, estructura, diseño y responsabilidades sobre textos e imágenes.', 'Desarrollamos, probamos en móvil y verificamos los recorridos de contacto.', 'Coordinamos publicación, accesos y mantenimiento según lo contratado.'],
    evidence: 'En el proyecto Más Servicios, el alcance documentado incluye un sitio WordPress autogestionable, servicios, sucursales, mapas, FAQ, formularios, WhatsApp y capacitación. Podés revisar su ficha y otros trabajos comerciales, personales y académicos.',
    evidenceLink: '/proyectos#mas-servicios',
    faqs: [
      ['¿Cuánto cuesta desarrollar una web?', 'El precio depende de las páginas, el contenido, la gestión interna y las integraciones. La home publica una referencia de desarrollo web desde USD 200; el presupuesto final requiere definir el alcance. Dominio, hosting, mantenimiento y servicios externos deben quedar detallados en la propuesta.'],
      ['¿Puedo actualizar la web por mi cuenta?', 'Sí, cuando el proyecto incluye un gestor de contenidos. Si solo necesitás cambios ocasionales, una solución estática puede simplificar el mantenimiento. Lo decidimos según quién va a editar y con qué frecuencia.'],
      ['¿La web queda preparada para Google Ads?', 'Podemos preparar una landing específica, sus llamadas a la acción y los eventos de medición. La publicación de campañas y la inversión publicitaria se acuerdan por separado.']
    ],
    related: ['google-ads', 'sistemas-crm']
  },
  {
    slug: 'marketing-digital', name: 'Marketing digital',
    title: 'Marketing digital para empresas | Reac Studio',
    description: 'Estrategia, canales, campañas y medición para ordenar el marketing de tu negocio. Reac Studio trabaja con empresas de Argentina y LATAM de forma remota.',
    h1: 'Marketing digital para conectar tu oferta con clientes',
    intro: 'Reac Studio ayuda a negocios a ordenar su estrategia digital: qué ofrecer, a quién, por qué canales y cómo medir las consultas que llegan. Conectamos marketing, web y seguimiento comercial en un mismo plan de trabajo.',
    fit: 'Tiene sentido revisar la estrategia cuando publicás sin un objetivo, usás varios canales sin coordinación o no sabés cuáles generan oportunidades. El primer paso es entender tu oferta, tus márgenes, tu capacidad de atención y cómo decide comprar tu cliente.',
    deliverables: [
      ['Diagnóstico y prioridades', 'Revisamos la presencia digital, los mensajes, los puntos de contacto y los datos disponibles. Priorizamos acciones según su relación con el objetivo y los recursos del negocio.'],
      ['Estrategia y ejecución', 'Definimos público, propuesta, canales y mensajes. Según el alcance, el plan puede incluir comunicación, campañas de publicidad y mejoras de la web para sostener el recorrido de compra.'],
      ['Medición y seguimiento', 'Acordamos qué cuenta como consulta y cómo registrar su calidad. Los clics aportan contexto; para evaluar el trabajo también necesitamos saber qué pasa después del contacto.']
    ],
    decision: 'Marketing digital abarca la estrategia y la coordinación de canales. Google Ads y Meta Ads son herramientas de publicidad dentro de esa estrategia. Antes de sumar canales, revisamos si existe una oferta clara, una página que la explique y una persona que pueda responder las consultas.',
    process: ['Revisamos negocio, oferta, público y situación actual.', 'Definimos prioridades, canales y criterios de medición.', 'Ejecutamos las acciones acordadas y coordinamos el contacto con tu equipo.', 'Revisamos resultados y ajustamos mensajes, inversión y recorridos.'],
    evidence: 'Tomás Ortiz trabaja en Marketing & Growth; Misael Ledesma, en Desarrollo Web & Tecnología. Esta combinación permite coordinar la comunicación con la web y las integraciones. Conocé los roles y la información del equipo publicada en Reac.',
    evidenceLink: '/nosotros',
    faqs: [
      ['¿Cuánto cuesta contratar marketing digital?', 'La home publica una referencia de gestión desde USD 150 por mes. El alcance y la cantidad de canales se definen en una propuesta. La inversión en anuncios de Google o Meta se paga aparte.'],
      ['¿Tengo que contratar todos los servicios?', 'No. Podés empezar por una necesidad concreta. Si detectamos que otra parte del recorrido está limitando el resultado, lo explicamos y priorizamos con vos.'],
      ['¿Cómo se evalúa el resultado?', 'Definimos indicadores vinculados al objetivo, como consultas recibidas y su calidad. Para relacionarlos con ventas necesitamos información del seguimiento comercial; la atribución no siempre permite asignar cada venta a un único canal.']
    ],
    related: ['google-ads', 'meta-ads']
  },
  {
    slug: 'google-ads', name: 'Google Ads',
    title: 'Gestión de Google Ads para empresas | Reac Studio',
    description: 'Campañas de Google Ads alineadas con búsquedas, anuncios y landing pages. Revisá tu captación de consultas con Reac Studio, en Argentina y LATAM.',
    h1: 'Gestión de Google Ads para captar búsquedas relevantes',
    intro: 'En Reac Studio trabajamos las campañas de Google Ads junto con la página de destino y la medición de consultas. El objetivo es que una búsqueda relacionada con tu servicio encuentre un anuncio y una respuesta coherentes.',
    fit: 'Google Ads puede ser útil cuando las personas ya buscan una solución como la tuya. Antes de lanzar revisamos oferta, zona de atención, presupuesto, demanda y capacidad para responder. Si todavía no está claro qué vendés o cómo se concreta una consulta, conviene resolver ese recorrido primero.',
    deliverables: [
      ['Búsquedas y estructura', 'Organizamos campañas alrededor de servicios e intenciones distintas. La revisión incluye palabras clave, términos de búsqueda y exclusiones para reducir consultas ajenas al alcance del negocio.'],
      ['Anuncios y landing', 'Alineamos el mensaje del anuncio con una página específica, beneficios concretos y una llamada a la acción. Una búsqueda sobre un servicio no debería obligar al visitante a descifrar toda la home.'],
      ['Medición de consultas', 'Distinguimos clics de contacto de formularios aceptados. Acordamos qué eventos usar para optimizar y cómo tu equipo identificará las oportunidades que realmente tienen sentido comercial.']
    ],
    decision: 'La gestión de campañas y el presupuesto que recibe Google son conceptos separados. También hay que definir si la landing ya existe, quién la mantiene y si necesita cambios. Una estimación responsable requiere estos datos; no prometemos un costo por cliente sin evidencia de tu negocio.',
    process: ['Revisamos cuenta, oferta, búsquedas y página de destino.', 'Acordamos estructura, presupuesto y eventos válidos.', 'Configuramos o ajustamos anuncios y verificamos el recorrido de contacto.', 'Analizamos términos, calidad de consultas y oportunidades de mejora.'],
    evidence: 'Podés empezar con la checklist de auditoría de Google Ads disponible en Reac. Reúne preguntas para revisar objetivos, estructura, palabras clave y medición antes de cambiar la inversión.',
    evidenceLink: '/recursos/auditoria-google-ads.html',
    faqs: [
      ['¿La inversión en Google está incluida?', 'No. La inversión publicitaria es independiente de la gestión. El presupuesto y el alcance del trabajo se detallan en la propuesta.'],
      ['¿Necesito una landing nueva?', 'Depende de la página actual. Revisamos si responde a la búsqueda, carga bien en móvil y permite consultar sin fricciones. Si cumple esos requisitos, podemos aprovecharla.'],
      ['¿Un clic en WhatsApp equivale a un cliente?', 'No. Indica intención de contacto, pero no confirma un mensaje, una oportunidad calificada o una venta. Conviene medir cada paso por separado y contrastarlo con el seguimiento comercial.']
    ],
    related: ['desarrollo-web', 'marketing-digital']
  },
  {
    slug: 'meta-ads', name: 'Meta Ads',
    title: 'Meta Ads: publicidad en Instagram y Facebook | Reac Studio',
    description: 'Estrategia y gestión de Meta Ads para negocios. Conectamos oferta, anuncios de Instagram y Facebook, destino y seguimiento de consultas con Reac Studio.',
    h1: 'Publicidad en Instagram y Facebook con una oferta clara',
    intro: 'Reac Studio trabaja campañas de Meta Ads para presentar una oferta, generar interés y facilitar consultas. Conectamos la creatividad del anuncio con el destino elegido y con la capacidad real del negocio para atender los mensajes.',
    fit: 'Meta Ads puede ayudar cuando necesitás que nuevas personas conozcan tu propuesta, especialmente si podés mostrar el producto, explicar un problema o demostrar cómo funciona un servicio. La oferta, las piezas y el seguimiento importan tanto como la configuración de la campaña.',
    deliverables: [
      ['Oferta y mensajes', 'Definimos qué proponés, para quién y por qué debería interesarle. Ordenamos los materiales disponibles y las variantes de mensaje que vale la pena probar sin inventar testimonios ni resultados.'],
      ['Campaña y destino', 'Seleccionamos un recorrido acorde al objetivo: mensajes, formulario o web. Acordamos los datos que necesitás recibir y quién responde, para que la campaña tenga continuidad después del clic.'],
      ['Evaluación de oportunidades', 'Revisamos desempeño de anuncios y calidad del contacto. Si un anuncio genera mensajes poco relevantes, investigamos el público, la promesa y las preguntas del formulario antes de aumentar inversión.']
    ],
    decision: 'Google Ads suele partir de una búsqueda explícita. En Instagram y Facebook el anuncio aparece mientras la persona consume contenido, por lo que la pieza necesita explicar la propuesta y captar su interés. La elección depende del negocio; no hay una plataforma ganadora para todos los casos.',
    process: ['Revisamos oferta, cuentas, materiales y atención de consultas.', 'Definimos mensajes, destino y criterios para evaluar oportunidades.', 'Preparamos las campañas y verificamos enlaces y formularios.', 'Revisamos variantes, frecuencia y calidad del seguimiento.'],
    evidence: 'La checklist de auditoría de Meta Ads de Reac permite revisar campañas de Facebook e Instagram con preguntas sobre objetivos, anuncios y resultados. Es un recurso de diagnóstico, no una promesa de rendimiento.',
    evidenceLink: '/recursos/auditoria-meta-ads.html',
    faqs: [
      ['¿Pueden enviar consultas a WhatsApp?', 'Podemos evaluar ese recorrido si tu equipo puede responder y registrar el seguimiento. La configuración depende de las cuentas y activos disponibles.'],
      ['¿Tengo que producir videos o imágenes?', 'Necesitamos materiales que expliquen tu oferta de manera auténtica. La producción y adaptación de piezas deben acordarse dentro del alcance; no se asumen incluidas automáticamente.'],
      ['¿La gestión incluye el presupuesto de anuncios?', 'No. La gestión y la inversión en Meta se cotizan como conceptos separados. También se define el alcance de piezas, medición y página de destino.']
    ],
    related: ['marketing-digital', 'google-ads']
  },
  {
    slug: 'automatizacion-ia', name: 'Automatización con IA',
    title: 'Automatización con IA para empresas | Reac Studio',
    description: 'Automatización de procesos, asistentes e integraciones para negocios. Reac Studio evalúa dónde la IA aporta valor y cómo mantener control sobre los datos.',
    h1: 'Automatización con IA para reducir tareas repetitivas',
    intro: 'En Reac Studio analizamos procesos del negocio y conectamos herramientas para reducir trabajo manual. Usamos inteligencia artificial cuando aporta valor, con límites claros, revisión humana y un recorrido definido para los errores.',
    fit: 'Conviene automatizar una tarea cuando se repite, tiene entradas reconocibles y podés describir qué significa hacerla bien. Si el proceso cambia todos los días o todavía nadie es responsable del resultado, primero hay que ordenarlo. Una regla simple puede resolver mejor el problema que un modelo de IA.',
    deliverables: [
      ['Mapa del proceso', 'Identificamos disparadores, datos, herramientas, responsables y excepciones. Elegimos una primera tarea acotada para comprobar si la automatización ahorra trabajo sin trasladar el problema a otra persona.'],
      ['Integraciones y asistentes', 'El alcance puede incluir clasificación de consultas, preparación de respuestas, conexión de formularios o seguimiento de leads. Las acciones sensibles requieren reglas de aprobación y permisos acordes a su impacto.'],
      ['Control y mantenimiento', 'Definimos registros operativos, alertas y una alternativa manual. Los servicios externos pueden cambiar o fallar; la solución necesita límites de uso, manejo de errores y un responsable de mantenimiento.']
    ],
    decision: 'Una automatización basada en reglas es adecuada cuando las condiciones son previsibles. La IA puede ayudar con texto variable, clasificación o borradores, pero sus respuestas deben validarse según el riesgo. Revisamos qué información se envía a cada proveedor y evitamos incorporar datos sensibles sin necesidad.',
    process: ['Elegimos una tarea y describimos su funcionamiento actual.', 'Definimos datos permitidos, reglas, límites y participación humana.', 'Construimos un flujo acotado y probamos también errores y excepciones.', 'Acordamos la puesta en marcha, medición y mantenimiento.'],
    evidence: 'En proyectos podés revisar ejemplos documentados de sistemas e integraciones y distinguir su carácter comercial, personal o académico. Esa evidencia permite conversar sobre alcance; no equivale a una métrica de ahorro validada para tu negocio.',
    evidenceLink: '/proyectos',
    faqs: [
      ['¿Qué tareas de una PyME conviene automatizar?', 'El registro de consultas, avisos internos, clasificación inicial y preparación de reportes son candidatos cuando existen datos y reglas claras. Empezamos por una tarea frecuente, de bajo riesgo y fácil de comprobar.'],
      ['¿Pueden crear un asistente para mi negocio?', 'Podemos evaluar un asistente con fuentes de información acotadas y derivación a una persona. Primero definimos qué puede responder, qué debe evitar y cómo revisar su calidad.'],
      ['¿Cuánto cuesta una automatización?', 'La home publica una referencia desde USD 250 por proyecto. El alcance depende de las herramientas y excepciones del proceso. Consumo de APIs, licencias y mantenimiento deben presupuestarse por separado cuando correspondan.']
    ],
    related: ['sistemas-crm', 'desarrollo-web']
  },
  {
    slug: 'sistemas-crm', name: 'Sistemas, CRM e integraciones',
    title: 'Sistemas y CRM para empresas | Reac Studio',
    description: 'CRM, sistemas internos e integraciones para ordenar clientes y operaciones. Reac Studio prioriza herramientas existentes y desarrollo a medida cuando hace falta.',
    h1: 'Sistemas y CRM para ordenar el seguimiento de tu negocio',
    intro: 'Reac Studio conecta sitios, herramientas y procesos comerciales para que tu equipo pueda trabajar con información organizada. Evaluamos un CRM existente, integraciones o desarrollo a medida según las necesidades reales del negocio.',
    fit: 'Un CRM empieza a ser útil cuando las consultas se reparten entre mensajes y planillas, se pierde el próximo paso o nadie sabe quién está siguiendo una oportunidad. Antes de construir un sistema, definimos qué información hace falta, quién puede verla y cómo se actualiza.',
    deliverables: [
      ['Proceso comercial y datos', 'Definimos clientes, contactos, oportunidades, estados y responsables. Acordamos qué campos son necesarios, qué datos pueden repetirse y cómo conservar un historial útil sin almacenar información de más.'],
      ['Herramientas e integraciones', 'Revisamos si una solución existente cubre el proceso. Cuando hace falta, conectamos formularios, web y otras herramientas o desarrollamos módulos específicos. El objetivo es evitar cargas duplicadas y pasos manuales innecesarios.'],
      ['Accesos y operación', 'El alcance de un sistema debe contemplar permisos, validaciones, copias de seguridad y manejo de errores. También definimos exportación de datos y responsabilidades para que el negocio pueda operar y mantener la solución.']
    ],
    decision: 'Adaptar un CRM existente suele acelerar el inicio cuando el proceso es estándar. Un desarrollo propio tiene sentido si hay reglas o integraciones que las herramientas disponibles no resuelven razonablemente. También implica mantenimiento, seguridad y costos de evolución que deben considerarse desde el comienzo.',
    process: ['Relevamos el proceso y las herramientas que ya usás.', 'Comparamos reutilizar, integrar o desarrollar, junto con sus costos.', 'Implementamos el alcance acordado y probamos permisos y flujos de trabajo.', 'Preparamos migración, capacitación y continuidad operativa según el proyecto.'],
    evidence: 'El portfolio incluye un CRM Comercial identificado como proyecto académico y un Panel Administrativo Modular identificado como proyecto comercial. Las fichas describen el desafío, la solución y el alcance documentado, sin atribuir métricas de negocio no verificadas.',
    evidenceLink: '/proyectos#crm-comercial',
    faqs: [
      ['¿Qué es un CRM?', 'Es una herramienta para organizar relaciones comerciales: datos de clientes, consultas, responsables y próximos pasos. Su utilidad depende de que el equipo lo use con un proceso claro.'],
      ['¿Necesito un CRM personalizado?', 'No necesariamente. Primero revisamos opciones existentes. El desarrollo a medida se justifica cuando aporta una capacidad necesaria que no puede cubrirse de forma razonable con configuración o integraciones.'],
      ['¿Se puede conectar la web con el CRM?', 'Sí, si el CRM ofrece una API, un webhook u otro mecanismo compatible. Revisamos permisos, validación de datos, duplicados y qué ocurre cuando alguno de los sistemas no está disponible.']
    ],
    related: ['automatizacion-ia', 'desarrollo-web']
  }
];
