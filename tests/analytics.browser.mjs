...
try{
  ...
  await page.goto(origin+'/contacto?email=private@example.test&utm_source=google&utm_campaign=search_web&gclid=test-click-id');
  assert.equal(gtagRequests,0);assert.equal(events.length,0);
  assert.equal(await page.locator('[name=utm_source]').inputValue(),'google');assert.equal(await page.locator('[name=utm_campaign]').inputValue(),'search_web');assert.equal(await page.locator('[name=gclid]').inputValue(),'test-click-id');
  ...
  assert.equal(await page.locator('[name=utm_source]').inputValue(),'google');assert.equal(await page.locator('[name=utm_campaign]').inputValue(),'search_web');
  ...
  assert.equal(events.filter(args=>args[1]==='contact_form_submit').length,1);assert.equal(events.filter(args=>args[1]==='contact_form_success').length,0);
  formStatus=200;await page.locator('[data-submit-button]').click();await page.waitForURL(origin+'/gracias');
  assert.equal(events.filter(args=>args[1]==='contact_form_submit').length,2);assert.equal(events.filter(args=>args[1]==='contact_form_success').length,1);
  const successful=events.find(args=>args[1]==='contact_form_success');assert.equal(successful[2].utm_source,'google');assert.equal(successful[2].utm_campaign,'search_web');
  ...
  await page.reload();assert.equal(events.filter(args=>args[1]==='contact_form_success').length,1);
  await page.goto(origin+'/google-ads');
  assert.ok(events.some(args=>args[1]==='service_view'&&args[2].service==='google-ads'));
  ...
  assert.ok(events.some(args=>args[1]==='cta_click'&&args[2].service==='google-ads'));
  ...
  await page.evaluate(()=>window.ReacAnalytics.track('diagnostic_click'));
  ...
  console.log('Analytics fixtures passed: consent, campaign attribution, one script per page, no PII, form attempt/success, service view/CTA and withdrawal.');
